...
def fix_dead_pixels_with_gaussian(image, threshold=1.5, sigma=1.0):
    """ 
    Fix isolated dead pixels using Gaussian filter. 
    Pixels brighter than 1.5 times their neighbors' average are replaced with Gaussian-weighted average.
    """
    # Create a copy of the image to avoid modifying the original during the loop
    fixed_image = image.copy()
    
    # Apply Gaussian filter to the entire image
    ...
    for i in range(1, image.shape[0] - 1):
        for j in range(1, image.shape[1] - 1):
            ...
            if pixel_value > threshold * neighbor_avg:
                # Replace the dead pixel with the Gaussian-weighted average of the surrounding pixels
                fixed_image[i, j] = smoothed_image[i, j]
...
def process_image(science_image, master_bias, master_flat):
    ...
    # Fix dead pixels with Gaussian smoothing
    final_image = fix_dead_pixels_with_gaussian(destriped_image)
...
output_dir = '/Users/yasir/Documents/20240312/processed'
